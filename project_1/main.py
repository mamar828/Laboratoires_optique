...
def compute_mean_motion_energy(video_name):
    ...
    motion_energy = video.get_mean_motion_energy()
    cutted_motion_energy = Video.get_filtered_array(motion_energy, 0.25 * (np.max(motion_energy) - np.min(motion_energy)) + np.min(motion_energy))
    plot(cutted_motion_energy, f"project_1/figures/{video_name.split('.')[0]}_filtered.png")


compute_mean_motion_energy("25_degree")
